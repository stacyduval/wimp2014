...
if ( ! function_exists( 'wimp_paging_nav' ) ) :
/**
 * Display navigation to next/previous set of posts when applicable.
 */
function wimp_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Posts navigation', 'wimp' ); ?></h1>
		<div class="nav-links">
...
if ( ! function_exists( 'wimp_post_nav' ) ) :
/**
 * Display navigation to next/previous post when applicable.
 */
function wimp_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		...
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">&larr;</span> %title', 'Previous post link', 'wimp' ) );
				next_post_link(     '<div class="nav-next">%link</div>',     _x( '%title <span class="meta-nav">&rarr;</span>', 'Next post link',     'wimp' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function wimp_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'wimp' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'wimp' ),
...
function wimp_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'wimp_categories' ) ) ) {
		...
		set_transient( 'wimp_categories', $all_the_cool_cats );
	}
...
function wimp_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'wimp_categories' );
}
